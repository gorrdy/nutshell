...
def find_env_file():
    ...
    if os.path.isfile(env_file):
        # override=False: real env vars (docker -e, compose env_file/environment, systemd Environment=) must beat the on-disk file, matching pydantic-settings precedence.
        env.read_env(env_file, recurse=False, override=False)
    else:
        env_file = ""
    return env_file
